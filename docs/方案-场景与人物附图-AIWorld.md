...
> **状态：未落地 · 备查用。** 2026-09-19 定方案，2026-09-23 补核源码，2026-09-24 **补齐图片规格（像素 / 存放位置 / 取图链路）+ 服务端归一化（Pillow 已装）**。**未写任何代码。**
> **来源**：`MEMORY-DETAIL.md` §16 / §17A（原始论证），本文是**可直接开工的落地版**。
> ⚠️ **行号为 2026-09-23/24 复核时的值（HEAD = `4e2a44c` + 三份方案未提交）。落地前按「锚点字符串」定位，不要照抄行号。**
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| **A** | 字段 + 上传口（含 Pillow 归一化）+ 取图口 + 界面 + 显示 | ✅ 可开工（本文已把坑填平） |
...
| 显示位 | 位置 | 尺寸 | 说明 |
|---|---|---|---|
| **场景大图** | 顶栏「场景」点开的**场景卡** | 宽 100%（336px）× `aspect-ratio: 16/9` | 有图才渲染这一块；圆角 10px |
| **在场一行** | 同一张场景卡的「在场」段 | **64×64** 圆形头像 + 名字；**每行 4 个**（`grid-template-columns: repeat(auto-fill, minmax(72px, 1fr))`，`gap: 6px`），超出换行 | **无图 = 同尺寸的文字格**（只有名字），**不画剪影**（§2 底座 2） |
| **工作台附图槽·场景** | 「场景」页每张卡内 | 预览 **240×135** | 上传 / 换图 / 移除 |
| **工作台附图槽·人物** | 「人物」页每张卡内 | 预览 **120×120** | 同上 |

**场景卡浮层规格**（新建，`#scene-card`）：

- **宽 360px**（与右侧抽屉 `.drawer` 同宽，视觉一致；`max-width: calc(100vw - 24px)`，移动端近全宽），内边距 12px ⇒ **内容宽 336px**。
- 结构：① 大图（有图才渲染）→ ② 场景名 + `perceivable` → ③「在场」一行 → ④「最近去过」（复用已有 `#scene-nav` 的 `data.recent_scenes`）。
...
| **显示位置** | **顶栏「场景」点开 → 场景卡**：大图 + `perceivable` + 在场一格 + 最近去过（已存在的 `#scene-nav`） |
