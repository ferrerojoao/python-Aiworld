...
"states": [
  { "id": "st_3f2a91c04d7e",                 // 状态 id：移除靠它精确定位（"骨裂"vs"左臂骨裂"按原文匹配必误伤）
    "text": "沐浴龙血：普通刀剑伤不了他",     // 一行事实，直接进提示词
    "since": "2026-07-20T03:10",             // 获得时的世界钟
    "source_event": "ev_00042",              // 哪条正文给的 → 可追溯 / 可撤销
    "public": true,                          // 表现是否对外可见（决定 NPC Actor 拿不拿）
...
| 函数 | 改动 | 口径 |
|---|---|---|
| `character_block()` | 主角段后加「状态」行（**恒在**）；在场 NPC 同理 | 全知：`text` + 成因 |
| `build_audit_work_order()` | 新增状态段 | 全知（否则判出与状态自相矛盾的正文） |
| `build_actor_work_order()` | 新增，**只取 `public=true`** | 只给表现、不给成因 |
...
- **移除**（审计 `state_remove`）：审计**看着带 `[st_xxx]` 的清单**按 id 报，找得到才删（找不到 → `skipped`，不误删近义条目）。
...
| 世界工作台手填 | ❌ 未做（Step 3） |
| NPC Actor 那一路注入 | ❌ 未接——没接就没有泄漏风险；接的时候必须同时实现 `public` 过滤，`tests/test_states.py::test_actor_view_does_not_leak_states` 是这条的哨兵 |
