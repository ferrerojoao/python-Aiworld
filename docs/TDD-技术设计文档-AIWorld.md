...
- **装配实现（2026-09-05 定稿、2026-09-10 重排：`app/core/workorder.py`）**：工作单 = 客观层（账本快照，纯函数块）+ 主观层（角色契约：身份/金科玉律/输出格式）两部分，`build_work_order(viewer, …)` 装配。**拼接顺序（2026-09-10 定稿）**：A 身份与任务 → C 金科玉律（禁忌前置，紧跟身份）→ J 编剧准则 → B 世界硬规则（资料区前段，永不裁剪）→ I 世界书命中（本场相关背景）→ D 角色资料 → H 幕后注 → F 场景快照（含在场目击时刻）→ 在场 NPC 已知集 → G 剧情目标（近因区引导，贴近动笔）→ K 输出格式 → E 事件日志（最近原文在后）→ 玩家输入消息。重排原则：金科玉律紧跟身份使禁忌置于最高注意区；静态资料（硬规则/世界书/角色卡）聚集成区；写作指令（剧情目标/输出格式）下沉近因区贴近动笔。可裁块（世界书命中）仍在静态区尾部，token 预算超支时最先可裁；身份/硬规则/金科玉律不可裁。viewer 参数为未来 actor_<npc_id>（物理隔离）与 qc（参照子集）预留。
