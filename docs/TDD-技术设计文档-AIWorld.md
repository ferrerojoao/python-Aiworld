...
**不变量**：
- 人物表有且仅有一张 `is_player` 卡。`load_world` 在缺失时补一张占位卡（名 `主角`），保证 `world.player()` 恒有返回值——引擎处处以主角名为键，缺卡会静默失配。
- **主角不可删**：世界工作台的人物卡不给主角卡删除按钮（前端），`save_world_assets` 服务端再校验一次（非 1 张 → `ValueError` → HTTP 400，且校验先于落盘，原文件不受影响）。
...
- **镜子不给镜子**：主角自己不出现在"在场"列表里——`scene_description` 与 `/state.present` 都剔除主角（那是"还有谁在"）；工作单的在场名单里保留他并标注 `（玩家）`（Actor 要认出对话对象）。
- **改名 = 换人物表键**：`PUT /sessions/{sid}/player` 改 `name` 即换键（目标同学的名字或已存在则 400）；旧日志保持原名。
...
- **主角与 NPC 同权（2026-09-13）**：`present_at` 遍历 `world.npcs` 全部键，主角在表里 → 自然出现在在场名单里（开场事件即他的第一条位置事实；否则第一轮工作单的在场名单里没有主角）。需要"别把自己算进别人堆里"的地方各自剔除：`scene_description` 与 `/state.present` 剔主角（那是"还有谁在"）；工作单的在场名单保留他并标注 `（玩家）`。
...
- 常规到达：引擎按场景属性拼静态底稿（尽量零 LLM）——"这里是{名字}。{可感知区原文}。在场：{在场者名列表}"。在场列表剔主角（2026-09-13）：这是玩家自己的视野，不必告诉他自己在。无人时写"四下无人。"
...
GET    /api/sessions/{sid}/world            # 世界工作台（概览/世界书/场景/NPC/轴/事件）；npcs 里含主角卡（is_player）
PUT    /api/sessions/{sid}/world            # 就地编辑当前世界（单一真相源：写的就是 content/<world>/，即时生效）；两级闸门：不变量（恰好一张主角卡）失败→400 先于落盘，语义问题（start_scene 不在场景表等）→保存成功但返回 problems 由 UI 软警告（2026-09-13）
...
> 下面的表是当初的排期，S0–S7 均已落地。**当前回归基线 = `pytest`（平铺 `test_*.py`，全绿且覆盖率须过 90% 闸门）+ `cd web && npm test`（jsdom 前端冒烟）**；静态检查是 `python -m ruff check`（规则集在 `pyproject.toml`）。真 LLM 冒烟不在这里，在 `scripts/`（`smoke_turn_real.py` / `smoke_state_real.py` / `smoke_llm.py`）。
