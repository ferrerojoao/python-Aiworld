...
**为什么单列**：`event_log_block(limit=10, recent_full=3)` 只给编剧最近 10 条、更早压成 60 字摘要；`active_lore_ids` 每轮采纳时 `rebuild_active_lore` 清空重建（关键词命中才注入，`LORE_ACTIVE_CAP=5`）。两者都留不住"角色此刻是什么"这种**长期事实**。
...
| **导演 action（`state_add` / `state_revoke`，两步确认门）** | ✅ Step 2c |
| 世界工作台手填 | ❌ 未做（Step 3） |
...
- **`state_view` 由引擎侧生成**（`workorder.state_view()`，内部复用注入提示词的那套过滤 + 同一组 cap），面板据此渲染 → **面板上"编剧能看到哪几条"与提示词字面同源**，前端不自己算 cap，不会漂移。返回按 **主角 → 在场 → 不在场 → 已退场** 定序（排序规则只有一处），每条分 `visible` / `hidden` / `expired` 三个桶。
...
- 动作 `state_add`（`{npc_id, text, until?, public?}`）与 `state_revoke`（`{state_id}`，退路 `{npc_id, text}` 做**精确文本**匹配——不做模糊匹配，"骨裂"与"左臂骨裂"必须分得开）。
- 路由走 `request.app.state.turn_runner_factory(session)` 拿 runner，调 `Transaction.add_state()` / `revoke_state()`——**与抽屉里的 `×` 是同一条路径**：留痕形状、"（玩家设定）/（玩家撤销）"文案、`last_state_change` 记账完全一致，不是一个动作两套痕迹。
...
- 手动改动进 `last_state_change.added` / `revoked` → 左侧栏「本回合变化」立刻显示、可即时反悔（`last_state_change is None` 时不记，只落记账事件——那种情况左侧栏本来就没有分区可显示）。
