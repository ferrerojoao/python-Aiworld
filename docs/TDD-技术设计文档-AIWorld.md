...
// tags 约定：`region:xxx` = 地理归属（跨区域知识边界，§6 已知集）；无 region 标签 = 全域公共区
...
// tags 约定：`region:xxx` = 来属地/听域（NPC 已知集公开事件的筛选范围，§6）；
//   缺省 = 按其亲历事件发生地推导；推导为空 = 保守（不听说任何公开旧事）。
//   典型用例：青石镇本地人 tags 含 region:qingshi；来出差的外地人活动场景在青石镇
//   但 tags 仍填 region:home_city —— 他听说过家乡的旧事，没听说过青石镇的打架名声。
{
...
- **读接口（查询即真相）**：
  ...
  - `known_set(npc_id, scene_id)` → NPC 已知集（机械知识边界）：亲历（全量 by_participant，不随滑窗）∪ known_by 含己（by_knower，含改判）∪ 听域内公开事件。公开事件按 **NPC 听域**筛选而非当前场景区域：听域 = NPC 卡 `tags` 中的 `region:xxx`（来属地/听域）；卡上无 region 标签时按其亲历事件发生地推导；推导为空 = 保守空集（不听说任何公开旧事）。场景表 `tags` 以 `region:xxx` 声明事件发生地归属——当前场景无标签 = 全域（单区域内容包行为不变），事件发生地无标签 = 全域公共区；**location 查不到场景的一次性布景 = 哨兵 `adhoc`，不进任何听域的风闻通道**（宁可不闻，不错闻；在场者走亲历通道不受影响，亲历 ad-hoc 也不会把听域推导出 adhoc）。用途：本地人听说本区旧事；**出差外地人**活动场景在本地、但听域仍是家乡，本地打架名声对其不可见。写手已知集块与 QC 参照区共用此函数（单一事实源）；Actor 切片仍走 `experiences(self)`（口径一致）。
...
- **知情总纲「角色不是你」（2026-09-09 收敛）**：编剧案头资料（事件日志 / 幕后注 / 世界书）角色本人并不知道；角色开口前须核对话词是否在其自身已知范围内——亲历、来历（人物卡）、被告知（剧情内），**在场 NPC 以工作单「已知集」清单为准（§6）**。由此派生：公开旧事异地角色默认不知（轰动大事可作风闻）、幕后注/私密绝不出口（知情者坦白除外）、无卡即兴角色只知眼前。判断依据 = 已知集清单 + 事件日志各行标注的发生地/在场者/时间（**数据做重活，规则讲原理**：机械清单与标注提供证据，规则只讲原理，不 enumerate 特例）。金科玉律级禁令，与"位置是快照"纪律同构（提示词纪律 + 机械清单混合；时间锚等机制兜底默认不做）。
...
- **NPC 已知集块（2026-09-09，取代旧"近况块"）**：`known_set_block` 为每个在场 NPC 机械计算知识清单（`Ledger.known_set`，每 NPC 最近 5 条 + "另知本区公开旧事"兜底句），写手与 QC 参照区共用——**比对基线与写作材料完全一致**。金科玉律配套条款："在场 NPC 的知识以已知集清单为准，清单外的事（含日志中异地事）一律不知"。跨区域效果：新手村英雄事迹对王都 NPC 不可见，玩家自报家门时 NPC 将信将疑；同区域轰动事件自动进听域覆盖本区的 NPC 已知集（风闻免打标）；NPC 听域以人物卡 `region:xxx` 标签为准（缺省按亲历推导，空 = 保守），出差外地人因此听不到本地旧事。
- **规则化拼接的意义**：原料只取已记账字段——每轮概貌稳定、成本确定、不产生正文二次概括的幻觉漂移。
- **装配实现（2026-09-05 定稿：`app/core/workorder.py`）**：工作单 = 客观层（账本快照，纯函数块）+ 主观层（角色契约：身份/金科玉律/输出格式）两部分，`build_work_order(viewer, …)` 装配。拼接顺序按三条原则：**前置原则**（身份与硬规则、金科玉律禁忌在最前——模型开头注意力最强，且合一 agent 的泄漏禁令必须置于高注意区）、**动态最近原则**（事件日志 → 场景快照 → 在场 NPC 已知集等每回合变化的块集中在尾部，离玩家输入更近，防止"把旧事当新事"）、**近因原则**（输出 JSON 格式说明收尾，紧贴玩家输入消息；编剧准则是"怎么写"的规则，贴近动笔位置）。可裁块（已知集/世界书命中）集中于尾部，token 预算超支时最先可裁；身份/硬规则/金科玉律不可裁。viewer 参数为未来 actor_<npc_id>（物理隔离）与 qc（参照子集）预留。
- **在场名单附最后目击时刻（2026-09-09）**：场景快照块每名 NPC 附"（最后目击：X 小时前）"（`_rel_seen` 由其最后参与事件的 `at` 对当前时钟算相对时间，解析失败退化为无标注纯名单）。数据做重活——"位置是快照可能已过期"从抽象规则变成看得见的数字，写手自行权衡刚目击（放心写他在场）与久未见（考虑扑空/挪了地方）；金科玉律"位置是快照"条款同步引用该标注。账本层不变：快照仍只能被事件更新，扑空不落账（无目的地可记），NPC 位置保持"最后被看见的地方"直到下次事件。
...
- **participants 判定 = 基线继承 + 明确进出场修正（2026-09-09 改版，与 location 联动）**：玩家**未移动**时，审计以工作单给出的在场名单为默认基线，正文没有明确进出场就原样继承——代词指代（"她笑了"）视为仍在场，指代不清保守保留原名单；摊主/路人等叙事背景不进名单。玩家**移动**时，participants 重置为玩家 + 正文明确的同行者——留在原地的不同行（其位置快照自然停在原地，防瞬移）。改动前审计每回合从正文重推名单，正文不点名就丢人（忽隐忽现）且 private 事件的 known_by 随之漏料。机械兜底：commit 时玩家恒并入 participants（`transaction.py`，名单确定性排序）。
