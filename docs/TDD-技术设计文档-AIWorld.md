...
| 前端 | `web/tests/`：jsdom 跑真实 `index.html + app.js`（18 条冒烟，各对应一个真出过的 bug 或一条对外行为——"无世界"启动 / 工作台空态 / 「秘」标签判据 `public === false` / 事件日志的改判可见性（接口给有效名单 + `access_rejudged`）/ `?token=` 吸收与注入 / 无令牌时不发空 `Authorization` 头 / 落卡页 6 条：左栏 `待落卡 · N` 计的是人物+场景两数、抽屉两段并列且**默认全收起**、**没展开就一个 `/draft` 都不发**、展开后草稿预填 + 挂「AI 草稿 · 请核对」标记 + POST 正文恰为 `{appearance, persona}`、证据**折叠到点击才拉**、场景行名称只读 + 听域警告 + POST 打到 `locations` / 主角锁定 2 条：已开局 → **每张卡**的 `is_player` 框都 `disabled` + 主角姓名 `disabled` + 提示写明「重置世界」唯一出口；未开局 → 都可改、提示仍在教怎么换 / 左栏在场人物 3 条：`player_present=true` → 主角排第一并挂「主角」标，`=false` 且无人 → 只剩「无人」（**不许硬塞主角**），`=false` 但有别人 → 只列别人、不出现主角行）。CI 的 `frontend` job 跑。⚠️ jsdom **不能**给 `<select>` 类 bug 下结论（不实现 dirty value flag），那类仍靠人工走查。⚠️ 探针**只回标量/字符串**：jsdom 的 Array 原型与 Node 不同，`deepStrictEqual` 会把相等的两个数组判成不等 |
