...
def test_present_at_derives_from_narrative(session):
    ...
    ledger.append(ev)
    assert "朱明" in ledger.present_at("网吧")
    assert ledger.where_is("朱明")["location"] == "网吧"
...
def test_visible_to_respects_known_by(session):
    ...
    visible_to_zhuming = {ev["id"] for ev in ledger.visible_to("朱明")}
...
def test_retired_npc_absent_from_present_at(session):
    ...
    entity = ledger.save.entities.setdefault("朱明", EntityRuntime())
    entity.lifecycle = "retired"
    assert ledger.where_is("朱明")["location"] == "网吧"
    assert "朱明" not in ledger.present_at("网吧")
...
def test_index_maintained_incrementally(session):
    ...
    assert ledger.where_is("朱明")["location"] == "校门口"
    assert "朱明" not in ledger.present_at("网吧")
    assert "朱明" in ledger.present_at("校门口")
...
def test_cache_stats_counts_hits_and_misses(session):
    ledger = session.ledger
    assert ledger.where_is("朱明") is None
